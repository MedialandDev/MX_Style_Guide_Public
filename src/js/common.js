...
export function pageSelector(PageNumberView = 5) {
  ...
  function updateCurrentPage() {
    const currentPage = parseInt(new URL(document.location).searchParams.get('page'), 10) || 1;
    if (totalPage > 0) {
    ...
    if (totalPage > PageNumberView) {
      ...
      const startPageNumber = currentPage <= PageNumberCount
        ? 1
        : currentPage - PageNumberCount; // 當前頁面小於等於 PageNumberView 時為1, 其餘為與 PageNumberCount的相差
      // 狀態 - 顯示最後面前 n 個
